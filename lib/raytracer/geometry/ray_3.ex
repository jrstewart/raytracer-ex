defmodule Raytracer.Geometry.Ray3 do
  @moduledoc """
  This module provides a set of functions for working with three-dimensional rays represented by an
  origin point and a direction vector.
  """
  ...
  @doc """
  Computes the point at `distance` on `ray` returning a tuple `{:ok, point}` where `point` is the
  computed point. `{:error, :none}` is returned if `distance` is less than 0.
  """
  @spec point_at(t, float) :: {atom, Point3.t() | atom}
  def point_at(_, distance) when distance < 0, do: {:error, :none}

  def point_at(ray, distance),
    do: {:ok, ray.direction |> Vector3.multiply(distance) |> Vector3.add(ray.origin)}
