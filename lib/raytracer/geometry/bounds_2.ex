defmodule Raytracer.Geometry.Bounds2 do
  @moduledoc """
  This module provides functions for working with two dimensional axis-aligned
  bounding boxes.
  """
  ...
  def corner(%Bounds2{} = bounds, 0), do: bounds.min
  def corner(%Bounds2{} = bounds, 1), do: %Point2{x: bounds.max.x, y: bounds.min.y}
  def corner(%Bounds2{} = bounds, 2), do: %Point2{x: bounds.min.x, y: bounds.max.y}
  def corner(%Bounds2{} = bounds, 3), do: bounds.max
  ...
  def diagonal(%Bounds2{} = bounds), do: Point2.subtract(bounds.max, bounds.min)

  @doc """
  Pads `bounds` in all directions by the value of `delta`.
  """
  @spec expand(t(), number()) :: t()
  def expand(%Bounds2{} = bounds, delta) do
  ...
  def inside?(%Bounds2{} = bounds, %Point2{} = point) do
  ...
  def inside_exclusive?(%Bounds2{} = bounds, %Point2{} = point) do
    point.x >= bounds.min.x && point.x < bounds.max.x && point.y >= bounds.min.y &&
      point.y < bounds.max.y
  end
  ...
  def intersect(%Bounds2{} = bounds1, %Bounds2{} = bounds2) do
    %Bounds2{
  ...
  `bounds` by the given amount in each direction specified by `tx` and `ty`
  values.
  """
  @spec lerp(t(), number(), number()) :: Point2.t()
  def lerp(%Bounds2{} = bounds, tx, ty) do
    %Point2{
  ...
  @doc """
  Returns either `:x` or `:y` indicating the direction of the largest extent of
  `bounds`.
  """
  @spec maximum_extent(t()) :: atom()
  def maximum_extent(%Bounds2{} = bounds) do
  ...
  """
  @spec offset(t(), Point2.t()) :: {number(), number()}
  def offset(%Bounds2{} = bounds, %Point2{} = point) do
  ...
  def overlap?(%Bounds2{} = bounds1, %Bounds2{} = bounds2) do
  ...
  def union(%Bounds2{} = bounds1, %Bounds2{} = bounds2) do
    %Bounds2{
  ...
  def union(%Bounds2{} = bounds, %Point2{} = point) do
    %Bounds2{
