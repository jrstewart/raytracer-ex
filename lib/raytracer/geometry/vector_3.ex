defmodule Raytracer.Geometry.Vector3 do
  ...
  @doc """
  Computes the parallel and perpendicular components of `vector2` with respect to `vector1`.

  ## Examples

      iex> v1 = %Vector3{dx: 1.0, dy: 0.0, dz: 0.0}
      iex> v2 = %Vector3{dx: 0.0, dy: 1.0, dz: 0.0}
      iex> Vector3.decompose(v1, v2)
      {%Vector3{dx: 0.0, dy: 0.0, dz: 0.0}, %Vector3{dx: 0.0, dy: 1.0, dz: 0.0}}

  """
  @spec decompose(t(), t()) :: {t(), t()}
  def decompose(vector1, vector2) do
    normalized = normalize(vector1)
    parallel = multiply(normalized, dot(normalized, vector2))
    perpendicular = subtract(vector2, parallel)
